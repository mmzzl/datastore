...
from config import (
    ...
    UI_CHECK_WAIT_TIME, MAIN_LOOP_WAIT_TIME, IMAGES_DIR, AUTO_EGG_IMAGE
)
...
class ShanHaiJing:
    ...
    def check_egg_screen(self):
        ...
        try:
            ...
            time.sleep(UI_CHECK_WAIT_TIME)
            # if not self.ui_checker.needs_to_capture():
            #     self.logger.info("不需要收服")
            ...
            self.logger.info("识别到数据：%s", text_list)
            if not text_list:
