...
if all_data:
    ...
    if lg.error_code == '0':
        ...
        if rs.error_code == '0' and len(rs.data) > 0:
            ...
            # 将股票代码转换为统一格式
            df_date['code'] = df_date['symbol'].apply(lambda x: f"sh.{x.lower()}" if x.startswith('SH') else f"sz.{x.lower()}")
            ...
            print(f"有行业分类的股票数: {len(merged_df)}")
            ...
            print("\n涨幅榜 TOP10:")
            for i, sec in sector_stats.head(10).iterrows():
                print(f"  {i+1}. {sec[1]['industry']}: {sec[1]['avg_change_pct']:.2f}% | 股票数: {sec[1]['stock_count']}")
            
            print("\n跌幅榜 TOP10:")
            for i, sec in sector_stats.sort_values('avg_change_pct', ascending=True).head(10).iterrows():
                print(f"  {i+1}. {sec[1]['industry']}: {sec[1]['avg_change_pct']:.2f}% | 股票数: {sec[1]['stock_count']}")
