...
class BaostockCollectorHS3005min(BaseCollector):
    ...
    def get_data(
        ...
        if df.empty:
            return
        df.columns = ["date", "time", "symbol", "open", "high", "low",
...
class BaostockCollectorALL1d(BaostockCollectorHS3005min):
    def __init__(
        self,
        save_dir: [str, Path],
        start=None,
        ...
        """
        self.region = region
        super(BaostockCollectorALL1d, self).__init__(
            save_dir=save_dir,
            start=start,
            end=end,
            interval=interval,
    ...
    def get_all_a_shares_symbols(self) -> List[str]:
        all_stocks = []
        rs = bs.query_stock_basic()
        while (rs.error_code == "0") & rs.next():
            all_stocks.append(rs.get_row_data())
        stock_df = pd.DataFrame(all_stocks, columns=rs.fields)
        stock_df = stock_df[stock_df["type"] == "1"]
        stock_df = stock_df[stock_df["status"] == "1"]
        stock_df = stock_df[
            ~stock_df["code"].str.startswith(("sh.000", "sz.399"))]
        return sorted(stock_df["code"].tolist())

    def get_instrument_list(self):
        if self.region == "ALL":
            logger.info("get all A-shares symbols......")
            symbols = self.get_all_a_shares_symbols()
        else:
            logger.info("get HS300 symbols......")
            symbols = self.get_hs300_symbols()
        logger.info(f"get {len(symbols)} symbols.")
        return symbols
...
class Run(BaseRun):
    ...
    def download_data(
        ...
        ---------
            ...
            # get all A-shares 1d data
            $ python collector.py download_data --source_dir ~/.qlib/stock_data/source/all_1d_original --start 2022-01-01 --end 2022-01-30 --interval 1d --region ALL
        """
        super(Run, self).download_data(max_collector_count, delay, start, end,
    ...
    def normalize_data(
        ...
        """
        if qlib_data_1d_dir is None or not Path(
                qlib_data_1d_dir).expanduser().exists():
            raise ValueError(
