...
class IndustryStockFinder:
    ...
    def get_industry_data(self, csv_path: str = "D:/stock_industry.csv") -> pd.DataFrame:
...
class StockAnalyzer:
    ...
    def analyze(self, news_analysis: Dict, llm_client=None, top_n: int = 5) -> Dict:
        ...
        logger.info(f"开始分析买入机会: hot_sectors={hot_sectors}, hot_stocks={hot_stocks}")
    ...
    def _get_klines_with_indicators(self, symbols: List[str], days: int = 60) -> pd.DataFrame:
        ...
        for symbol in symbols:
            try:
                data = mongo.get_kline(symbol, start_date=start_date, end_date=end_date, limit=100)
                if data:
        ...
        result = result.rename(columns={'code': 'symbol', 'pct_chg': 'change_pct', 'turnover': 'turnover_rate'})
        
        numeric_cols = ['open', 'close', 'high', 'low', 'volume', 'amount', 'pct_chg', 'amplitude', 'turnover']
    ...
    def _filter_by_rules(self, df: pd.DataFrame, top_n: int) -> Dict:
        ...
        # 筛选条件
        conditions = (latest['change_pct'] > 0) & (latest['rsi'] < 70) & (latest['ma5'] > latest['ma10'])
        ...
        candidates['score'] = (
            ...
            ((candidates['ma5'] - candidates['ma10']) / candidates['ma10'] * 100) * 0.2 +
    ...
    def _analyze_with_llm(self, df: pd.DataFrame, news_analysis: Dict, top_n: int, llm_client) -> Dict:
        if df.empty:
