...
class IndicatorConfig(BaseModel):
    ...
    kdj: Dict[str, Any] = Field(default_factory=lambda: {
        "period": 9,
        "k_buy_level": 20,
        "k_sell_level": 80
    })
...
class MonitorConfig(BaseModel):
    ...
    indicators: IndicatorConfig = Field(default_factory=IndicatorConfig, description="指标配置")
...
class TechnicalData(BaseModel):
    ...
    macd: MACD = Field(default_factory=lambda: MACD(macd=0.0, signal=0.0, histogram=0.0))
    kdj: KDJ = Field(default_factory=lambda: KDJ(k=50.0, d=50.0, j=50.0))
    bollinger: BollingerBands = Field(default_factory=lambda: BollingerBands(upper=0.0, middle=0.0, lower=0.0))
...
class MonitorHistory(BaseModel):
    ...
    technical_data: Dict[str, Any] = Field(default_factory=dict, description="技术指标数据")
    reasons: List[str] = Field(default_factory=list, description="信号原因")
