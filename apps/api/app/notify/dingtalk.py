...
class DingTalkNotifier:
    ...
    def _ensure_akshare_client(self):
        ...
        if self.akshare_client is None:
            logger.warning(
                "AkshareClient is None, cannot initialize without target_date"
            )
            logger.warning(
                "Please ensure AkshareClient is passed to DingTalkNotifier constructor"
            )
    ...
    def send(
        ...
        for attempt in range(max_retries):
            ...
            params = (
                {"timestamp": timestamp, "sign": self._sign(timestamp)}
                if self.secret
                else {}
            )
            ...
            if markdown:
                payload = {
                    "msgtype": "markdown",
                    "markdown": {"title": f"盯盘预警", "text": markdown},
                    "at": {"isAtAll": at_all},
                }
            elif msg:
                payload = {
                    "msgtype": "markdown",
                    "markdown": {"title": f"📊 盘前信息 {date}", "text": msg},
                }
            else:
                return False
            ...
            try:
                resp = requests.post(
                    self.webhook_url, json=payload, params=params, timeout=10
                )
                if resp.status_code == 200:
                    result = resp.json()
                    if result.get("errcode") == 0:
                        logger.info(
                            f"DingTalk notification sent successfully for {date}"
                        )
                        return True
                    else:
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(
                                f"重试发送钉钉消息 ({attempt + 1}/{max_retries})..."
                            )
                            import time
